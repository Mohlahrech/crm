...
class CrmLead(models.Model):
    ...
    def action_create_activity(self):
        """Create three activities based on the latest call date + 30, 60, and 80 days, only if they haven't been created already."""
        ...
        for lead in self:
            ...
            # Check if activities already exist based on the latest call date
            ...
            # If activities exist for this lead and the latest call hasn't changed, skip creation
            if existing_activities:
                raise UserError("Les activités ont déjà été créées pour cette date de récuperation.")
    ...
    # def action_create_activity(self):
    #     """Create an activity based on the latest call date + 30 days."""
    #     activity_type_id = 2  # Set the activity type ID
    #
    #     for lead in self:
    #         if not lead.user_id:
    #             raise UserError("Please assign a responsible user before creating an activity.")
    #
    #         # Get the latest call date from call_ids safely handling False values
    #         latest_call = max(
    #             lead.call_ids.filtered(lambda r: r.date_recup),
    ...
    #
    #         if not latest_call:
    #             raise UserError("No valid call history available to determine the deadline.")
    #
    #         date_deadline = latest_call.date_recup + timedelta(days=30)
    #
    #         self.env['mail.activity'].create({
    #             'res_model_id': self.env['ir.model']._get_id('crm.lead'),
    #             'res_id': lead.id,
    ...
    #             'note': 'This activity is based on the last call date plus 30 days.',
    #         })



    # def action_create_activity(self):
    #     """Create an activity for the lead with specific conditions."""
    #     activity_type_id = 2  # Set the activity type ID
    #
    #     for lead in self:
    #         if not lead.user_id:
    ...
    #             'summary': 'Follow-up Activity',
    #             'note': 'This is an automated activity created for follow-up.',
    #         })
